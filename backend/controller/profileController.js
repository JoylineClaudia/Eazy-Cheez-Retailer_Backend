...
const getProfile = asyncHandler( async (req, res) => {
    ...
    try{
        const {_id, username, phone ,storeaddress, storecategory} = await Profile.findOne({ _id: req.params.id})
        res.status(200).json({
            _id: _id,
            username: username,
            phone: phone,
...
const updateProfile = asyncHandler( async (req, res) => {
    ...
    const {_id, username, phone,storeaddress, storecategory} = await Profile.findOneAndUpdate({_id: req.params.id}, req.body, { new: true })
    res.status(200).json({
        _id: _id,
        username: username,
        phone: phone,
